...
func Test_ServiceManager_GetServiceTarget_UnsupportedHost(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "service host 'missing-target' for service 'api' is not supported")
	require.Contains(t, err.Error(), strings.Join(builtInServiceTargetNames(), ", "))
}
