...
func (s *AiModelService) ListModels(
	...
	if len(locations) == 0 {
		subLocations, err := s.subManager.ListLocations(ctx, subscriptionId)
		if err != nil {
...
func (s *AiModelService) ListFilteredModels(
	...
	if options != nil && len(options.Locations) > 0 {
		locations = options.Locations
	}

	models, err := s.ListModels(ctx, subscriptionId, locations)
	if err != nil {
		return nil, err
	}
...
func (s *AiModelService) resolveDeployments(
	...
	if quotaOpts != nil && len(options.Locations) != 1 {
		return nil, fmt.Errorf(
			"quota checking requires exactly one location, got %d", len(options.Locations))
	}
	...
	if len(results) == 0 {
		return nil, fmt.Errorf("no deployment found for model %q with the specified options", modelName)
	}
