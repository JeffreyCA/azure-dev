...
func buildNamespaceArgs(foundCmd *cobra.Command, remainingArgs []string) []string {
	...
	// Append remaining non-flag args
...
func tryAutoInstallForPartialNamespace(
	...
	// If the found command is an extension command (has extension.id annotation),
	// the extension is already installed and will handle remaining args itself.
	// Extension commands use DisableFlagParsing so they process all args internally.
	if _, isExtensionCmd := foundCmd.Annotations["extension.id"]; isExtensionCmd {
		return false
	}

	// Find the first non-flag argument (potential unrecognized subcommand)
	...
	// If no remaining args or the first arg is a valid subcommand, nothing to do
	if firstRemainingArg == "" || hasSubcommand(foundCmd, firstRemainingArg) {
		return false
	}

	// Build the full namespace from command path + remaining args
	...
	// Found matching extension(s) - trigger auto-install flow
	console.Message(ctx,
