...
// at the binary root so service-target providers and project lifecycle event
// handlers are registered once per binary, independent of which subcommand
// the user invoked (azd calls `<binary> listen` directly, not through any
// namespace prefix).
