...
func NewRootCommand() *cobra.Command {
	...
	// Framework commands live at the binary root so they don't depend on
	// which subcommand the user invoked. azd calls `<binary> listen` and
	// `<binary> metadata` directly, without going through any namespace path.
	rootCmd.AddCommand(azdext.NewListenCommand(agents.ConfigureExtensionHost))
