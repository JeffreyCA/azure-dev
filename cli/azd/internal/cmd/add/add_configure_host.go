...
var ServiceLanguageMap = map[project.ServiceLanguageKind]project.AppServiceRuntime{
	project.ServiceLanguagePython: {
		Type:    project.AppServiceRuntimeTypePython,
		Version: "3.13",
	},
	project.ServiceLanguageJavaScript: {
		Type:    project.AppServiceRuntimeTypeNode,
		Version: "22-lts",
	},
...
func addServiceAsResource(
	...
	if svc.Host == project.ContainerAppTarget {
		...
		if !ok {
			return nil, fmt.Errorf("unsupported service language: %s", svc.Language)
		}
		...
		confirm, err := console.Confirm(ctx, input.ConsoleOptions{
			Message: fmt.Sprintf("azd will use %s to host this project on %s. Continue?",
				output.WithHighLightFormat(runtime.Type.String()+" "+runtime.Version),
				color.MagentaString(svc.Host.String()),
			),
			DefaultValue: true,
		})
