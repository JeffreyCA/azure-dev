...
func (f *PublishFlags) BindNonCommon(local *pflag.FlagSet, global *internal.GlobalCommandOptions) {
	f.global = global
}
...
func isChildAction(ctx context.Context) bool {
	value, ok := ctx.Value("child-action").(bool)
	return ok && value
}
...
func (pa *PublishAction) Run(ctx context.Context) (*actions.ActionResult, error) {
	...
	pa.console.Message(ctx, fmt.Sprintf("Is Child Action: %t", isChildAction(ctx)))
