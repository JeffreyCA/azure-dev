...
func Test_CLI_Up_Down_FuncApp(t *testing.T) {
	...
	if session != nil {
		subscriptionID, ok := envValues[environment.SubscriptionIdEnvVarName].(string)
	...
	functionURI, ok := envValues["AZURE_FUNCTION_URI"].(string)
	require.True(t, ok, "AZURE_FUNCTION_URI should be a string in env values")
	url := fmt.Sprintf("%s/api/httptrigger", functionURI)
